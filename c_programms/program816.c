...
int leafnode(PNODE first)
{
    ...
    if(first!=NULL)
    {
        ...
        leafnode(first->lchild);
        leafnode(first->rchild);
    }
    return count;
}

int parentnode(PNODE first)
{
    static int count=0;
  if(first!=NULL)
  {
    ...
    parentnode(first->lchild);
    parentnode(first->rchild);
  }
  return count;
}
...
int main()
{
    ...
    iRet=leafnode(head);
    printf("total leaf node of tree %d",iRet);

    iRet=parentnode(head);
    printf("total parent nodes of tree %d",iRet);
    return 0;
}